<x-partial.modal :show="$show" formTitle="{{ $formTitle }}" modalSize="modal-lg">
  ...
  @endphp --}}
<x-partial.form action="{{ url('insert_monthly_report') }}">
    <x-partial.select selectLabel="Bulan" selectName="month" :selectValue="$monthYearSelect['month']" optValue="id" optText="name" :optUpdateValue="$month"/>
    ...
    @if ($edit)
    <x-partial.input type="hidden" name="report_id" :value="$monthlyReportId"/>
    @else
